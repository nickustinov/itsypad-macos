...
xcodebuild -scheme itsypad -configuration Release \
    ...
    -archivePath "$DIST_DIR/itsypad.xcarchive" \
    archive \
    ONLY_ACTIVE_ARCH=NO \
    CODE_SIGN_IDENTITY="" \
...
rm -rf "$APP_BUNDLE"
cp -R "$DIST_DIR/itsypad.xcarchive/Products/Applications/Itsypad.app" "$APP_BUNDLE"
rm -rf "$DIST_DIR/itsypad.xcarchive"
...
if security find-identity -v -p codesigning | grep -q "Developer ID Application"; then
    ...
    codesign --force --options runtime --deep --sign "$SIGNING_IDENTITY" \
        --entitlements "$PROJECT_DIR/Sources/itsypad.entitlements" \
        "$APP_BUNDLE"
